...
def api_system_reset_request():
    ...
    try:
        ...
        if not target_email:
            return jsonify({'success': False, 'error': 'No security email configured. Set CREDIFY_SECURITY_EMAIL.'}), 400
        try:
...
Credify Security Engine"""
            ...
            if not sent:
                ...
                db.session.commit()
                return jsonify({'success': False, 'error': 'Failed to send authorization email. Check SMTP settings and mailbox permissions.'}), 500
...
def api_system_reset():
    ...
    try:
        ...
        try:
            if target_email:
                report_sent = bool(mailer.send_nuke_report(
                    to_email=target_email,
                    stats=stats,
