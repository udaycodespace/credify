...
def activate_verify():
    ...
    if action == "confirm":
        ...
        setup_sent = mailer.send_setup_mail(
            user.email, user.full_name, degree, cid, token, student_id=user.student_id, year=year
        )
        ...
        return render_template("rejection_reason.html", full_name=user.full_name, token=token)
...
def api_forgot_password():
    ...
    try:
        ...
        user = User.query.filter(User.role == "student", User.student_id == raw_id).first()
        ...
        if not user:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": f'No student account found for roll number "{raw_id}". Please enter the exact roll number shown in your academic records.',
                    }
                ),
                404,
            )
        ...
        if not user.email:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "No registered email on file for this account. Please visit the Academic Records Office.",
                    }
                ),
                400,
            )
        ...
        sent = mailer.send_reset_password_mail(user.email, user.full_name, user.student_id, program, token)

        if sent:
            parts = user.email.split("@")
            masked = parts[0][:3] + "***@" + parts[1] if len(parts) == 2 else "***"
            return jsonify(
                {"success": True, "message": f"Password reset link sent to {masked}. Please check your inbox."}
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Failed to send recovery email. Please contact the Academic Records Office.",
                }
            ),
            500,
        )
...
def api_reset_password():
    ...
    try:
        ...
        if existing:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": f'Username "{new_username}" is already taken. Please choose a different one.',
                    }
                ),
                409,
            )
        ...
        return jsonify(
            {
                "success": True,
                "message": f'Credentials saved! Login with username "{new_username}" and your new password.',
            }
        )
    ...
    if request.method == "POST":
        ...
        if user and user.is_active:
            ...
            if portal_role and user.role != portal_role:
                ...
                flash(
                    f" Access Denied: This is the {portal_name} portal. Please login with a {portal_role} account.",
                    "danger",
                )
            ...
            if user.role == "issuer":
                if not mfa_token:
                    ...
                    if not target_email:
                        flash(
                            " MFA_CHALLENGE: No security email configured. Set CREDIFY_SECURITY_EMAIL or update issuer email.",
                            "danger",
                        )
                        return render_template("login.html", portal=portal_role)
                    ...
                    try:
                        sent = mailer.send_security_otp(to_email=target_email, full_name=user.full_name, otp=otp)
                        if sent:
                            flash(f" MFA_CHALLENGE: Enter the security code sent to {masked_email}.", "info")
                        else:
                            flash(
                                " MFA_CHALLENGE: Failed to deliver email OTP. Check SMTP settings or mailbox permissions.",
                                "danger",
                            )
                    except Exception as e:
                ...
                if not mfa_valid:
                    ...
                    return render_template(
                        "login.html", show_mfa=True, mfa_username=username, mfa_password=password, portal=portal_role
                    )
