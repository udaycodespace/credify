...
class User(db.Model):
    ...
    def get_totp_uri(self):
        ...
        try:
            ...
            return pyotp.totp.TOTP(self.totp_secret).provisioning_uri(name=self.username, issuer_name="Credify GPREC")
        except ImportError:
            return None
    ...
    with app.app_context():
        try:
            ...
            try:
                ...
                try:
                    db.session.rollback()
                    db.query_executor = db.session.execute(
                        text("ALTER TABLE users ADD COLUMN mfa_email_code VARCHAR(6)")
                    )
