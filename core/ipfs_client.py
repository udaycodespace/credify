...
class IPFSClient:
    ...
    def _add_to_ipfs(self, data):
        ...
        try:
            ...
            response = requests.post(f"{self.current_endpoint}/api/v0/add", files=files, timeout=30)
    ...
    def _get_from_ipfs(self, cid):
        ...
        try:
            response = requests.post(f"{self.current_endpoint}/api/v0/cat", params={"arg": cid}, timeout=30)
    ...
    def pin_content(self, cid):
        ...
        try:
            response = requests.post(f"{self.current_endpoint}/api/v0/pin/add", params={"arg": cid}, timeout=30)
            if response.status_code == 200:
    ...
    def _get_ipfs_stats(self):
        ...
        try:
            response = requests.post(f"{self.current_endpoint}/api/v0/stats/repo", timeout=10)
            if response.status_code == 200:
                return response.json()
        except Exception as e:
