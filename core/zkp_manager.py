...
class ZKPManager:
    ...
    # ==================== RANGE PROOF (GPA) ====================
    def generate_range_proof(self, credential_id, field_name, actual_value, min_threshold=None, max_threshold=None):
        """
        ...
        try:
            ...
            if not range_satisfied:
                return {"success": False, "error": f"Actual value does not satisfy the range requirement"}
    ...
    def verify_range_proof(self, proof, challenge_value=None):
        ...
        try:
            ...
            if challenge_value is not None and nonce is not None:
                ...
                recomputed = hashlib.sha256(f"{challenge_value}:{nonce}:{proof['credentialId']}".encode()).hexdigest()
    ...
    def generate_membership_proof(self, credential_id, field_name, full_set, claimed_member):
        """
        ...
        try:
            if claimed_member not in full_set:
                return {"success": False, "error": f"{claimed_member} is not in the {field_name} set"}
    ...
    def generate_set_membership_proof(self, credential_id, field_name, actual_value, allowed_set):
        """
        ...
        try:
            if actual_value not in allowed_set:
                return {"success": False, "error": f"Value not in allowed set for {field_name}"}
    ...
    def verify_set_membership_proof(self, proof, revealed_value=None):
        ...
        try:
            if revealed_value:
                ...
                nonce = proof["nonce"]
                recomputed = hashlib.sha256(f"{revealed_value}:{nonce}:{proof['credentialId']}".encode()).hexdigest()
